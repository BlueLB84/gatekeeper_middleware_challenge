...
function gateKeeper(req, res, next) {
  const usrnamePasswrd = queryString.parse(req.get('x-username-and-password'));
  function checkUSERS(usersObj) {
    if ((usersObj.userName === usrnamePasswrd.user) && (usersObj.password === usrnamePasswrd.pass)) {
      return usrnamePasswrd;
    } else {
      return null;
    }
  }
  req.user = USERS.find(checkUSERS);
  next();
}
